...
>
> **为什么先问 AI**：多数常见问题（面板找不到 / 数据为空 / 容器起不来 / 迁移失败 / 地址显示空 / 升级失败）AI 拿到日志能给出诊断方向，比开 issue 等回复快得多。AI 也判断不出再来 GitHub 开 issue，附上 AI 的诊断结论能让维护者跳过重复排查。
>
...
- 容器名（`docker exec` 后跟）：`teslamate-database-1` / `teslamate-grafana-1` / `teslamate-teslamate-1`
- service 名（`docker compose logs` 后跟）：database / grafana / teslamate / mosquitto
