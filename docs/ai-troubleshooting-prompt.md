...
a) **PostgreSQL 版本 < 18**（v1.7.0+ 硬要求）→ 仪表盘报 `function date_trunc(text, timestamp with time zone, text) does not exist` 或类似 SQL 错。本项目 12 个仪表盘用 3-arg `date_trunc`（PG 16+ 支持，但官方默认 PG 18）。**必须先升 PG 到 18**：备份 → 删旧 PG volume → 拉新 PG 18 → 恢复数据。流程见 TROUBLESHOOTING.md「PostgreSQL 大版本升级」章节。**不能跳过 PG 升级**，否则一半仪表盘空白。
   ⚠️ **先看 does not exist 的是哪个函数,两种因别混**：报 `lat_for_map` / `lng_for_map`（地图整页报错）或 `effective_cost` / `compute_tou_cost`（分时电价）等**自定义函数** does not exist → **不是 PG 版本问题,别去升级 PostgreSQL**,而是 **SQL 三件套没装 / 更新镜像后没重装**（坐标函数在 `install-coord-functions.sql`、分时电价在 `install-tou.sql`）。修复:重跑 SQL 三件套（命令见本问题 d 段，把 `install-tou` 换成全部三个 `install-coord-functions install-tou install-indexes`）。只有 `date_trunc(...)` does not exist 才是 PG<18,才需要升 PG。
