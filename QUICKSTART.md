...
| **B. 云服务器**（阿里云/腾讯云/AWS Lightsail 等，2GB 起） | 想用公网 IP 远程访问的用户 | ⭐⭐ ⚠️ [必读安全章节](#cloud-security) |
...
> 🎯 **国内用户强烈推荐方法 A**（手机 App 拿 Token，最简单），方法 B 更折腾但走的是 Tesla 官方 OAuth 流程。

#### ⭐ 方法 A：用「Auth for Tesla」App 拿 Token（推荐，国内最简单）

**为什么推荐：**
- 你手机上的 Tesla App 已经是登录状态 → 用同一个手机打开 Auth for Tesla → 一次完成授权，**不用配中国区 API、不用改 docker-compose.yml、不用重启服务**
- 国内 Tesla 群 / TeslaFi 用户群里早已是主流方案
- TeslaMate 拿到 Token 后会自动续期（用 refresh_token 换新的 access_token），你不用每天换
...
- Android：Google Play 搜「Auth for Tesla」/ 国内 Tesla 论坛找安装包

**2. App 内登录你的 Tesla 账号**

输入特斯拉账号密码，完成两步验证。App 会显示两段字符串：
...
浏览器打开 `http://服务器IP:4000`，登录页除了大按钮 `Sign in with Tesla` 外，下方有个折叠选项 `Use existing tokens` / `使用现有 Token`。点开后两个输入框：
- `Access Token` ← 粘贴 App 给你的 access_token
- `Refresh Token` ← 粘贴 App 给你的 refresh_token

点提交，**绑定完成**。跳过下面的方法 B。

> ⚠️ Auth for Tesla 是社区开源工具，**只在你信任的设备上用**（你的 iPhone / 自己的 Android）。它在本地完成登录，不会把密码上传到任何服务器。

---

#### 方法 B：TeslaMate 内置 OAuth（直接在网页登录）

> 国际账号 / 想用 Tesla 官方 OAuth 流程的用户走这条。**国内中国大陆账号需要先改 docker-compose.yml**，否则跳到 `auth.tesla.com` 登不上。

**默认流程（国际账号）：**
1. 点击 TeslaMate 页面上的 **「Sign in with Tesla」** 按钮
2. 跳到 `auth.tesla.com`（Tesla 官方登录页）
3. 输入特斯拉账号密码，过两步验证
4. 自动跳回 TeslaMate，绑定完成

**🇨🇳 中国大陆账号：必须先改配置，让登录跳到 `auth.tesla.cn`**
...
保存退出（nano: <kbd>Ctrl</kbd>+<kbd>O</kbd> 回车 → <kbd>Ctrl</kbd>+<kbd>X</kbd>），重启：
```bash
docker compose up -d
```
...
> - 两步验证超时 → 收到验证码立即输入，别拖
> - 网络不通 → 检查服务器能否访问 `auth.tesla.com` / `auth.tesla.cn`
> - 反复登不上 → **回头用方法 A**（Auth for Tesla App），多数情况手机端能成功登入
